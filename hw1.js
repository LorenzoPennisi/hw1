...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let images = document.querySelectorAll('#sliderHome img');
  let currentIndex = 0;
  let totalImages = images.length;

  // Funzione per nascondere tutte le immagini eccetto quella con l'indice specificato
...
//------------------------------------------------------------------------
  // Funzione per passare alla prossima immagine dopo un intervallo di tempo
  function nextImage() {
    ...
    // Ripeti la funzione dopo 3 secondi
    setTimeout(nextImage, 5000);
  }

  // Mostra la prima immagine all'avvio e avvia il ciclo delle immagini
...
function onJsonProdCons(json){
  console.log("Json Prod Cons Arrivato");
  console.log(json);

  const cont_prodotti = document.querySelector('.Prodotti');
...
function onJsonProd(json){
  ...
  console.log("Json Prod Arrivato");
  console.log(json);

  const contenitore = document.querySelector('.ProdottiInEvidenza');

  let contenitor_prodotti = document.createElement('div');
  contenitor_prodotti.classList.add('ProdottiInEvidenza1');
  contenitor_prodotti.innerHTML = '';

  contenitore.appendChild(contenitor_prodotti);

  array_prodotti2 = Object.values(json); // Converto l'oggetto in un array

  for(let i = 0; i <= 4; i++){
    prodotto = array_prodotti2[i];
    ...
    contenitor_prodotti.appendChild(itemsMain);
    itemsMain.appendChild(itemsMain2);
    itemsMain2.appendChild(itemsMain3);
  }

  contenitor_prodotti = document.createElement('div');
  contenitor_prodotti.classList.add('ProdottiInEvidenza1');
  contenitor_prodotti.innerHTML = '';
  ...
  for(let i = 5; i <= 9; i++){
    ...
    itemsMain = document.createElement('div');
    itemsMain.classList.add('itemsMain');

    itemsMain2 = document.createElement('div');
    itemsMain2.classList.add('itemsMain2');
    ...
    itemsMain3 = document.createElement('div');
    itemsMain3.classList.add('itemsMain3');
    ...
    ivaSpan = document.createElement('span');
    ivaSpan.textContent = '-Iva Inclusa';
    itemsMain3.appendChild(ivaSpan);
...
fetch("get_products_consigliati.php").then(onResponseProd).then(onJsonProdCons);
fetch("get_products.php").then(onResponseProd).then(onJsonProd)
...
function chiudiModale(event){
  const modal = document.getElementById('categModal');

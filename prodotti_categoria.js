...
//------STAMPA PROD CATEGORIA--------

function JsonProdCat(json){

  console.log("Json Prod Cons Arrivato");
  console.log(json);

  const cont_prodotti = document.querySelector('.Prod');
  cont_prodotti.innerHTML = '';
  const prodotti = Object.values(json); // Converto l'oggetto in un array
...
const prodotti_div = document.querySelector('.Prod');
const categoria = prodotti_div.getAttribute('data-categoria');
console.log(categoria);
fetch("get_prod_categoria.php?categoria=" + categoria).then(onResponseProd).then(JsonProdCat);
